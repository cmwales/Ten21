...
/// <summary>Business-layer refactor: extracted from UnitTiersController. No repository --
/// every query here is a single-table find/list. No interface -- same reasoning as
/// ChargeService/PaymentService.</summary>
public class UnitTierService
{
    private readonly Ten21DbContext _dbContext;
    private readonly IInputSanitizer _sanitizer;

    public UnitTierService(Ten21DbContext dbContext, IInputSanitizer sanitizer)
    {
        _dbContext = dbContext;
        _sanitizer = sanitizer;
    }

    public async Task<IReadOnlyList<UnitTierResponse>> GetUnitTiersAsync(CancellationToken cancellationToken) =>
        await _dbContext.UnitTiers.AsNoTracking()
            .OrderBy(t => t.TierName)
            .Select(t => ToResponse(t))
            .ToListAsync(cancellationToken);

    public async Task<UnitTierResponse> GetUnitTierAsync(Guid id, CancellationToken cancellationToken)
    {
    ...
    public async Task<UnitTierResponse> CreateAsync(UpsertUnitTierRequest request, CancellationToken cancellationToken)
    {
    ...
    public async Task<UnitTierResponse> UpdateAsync(Guid id, UpsertUnitTierRequest request, CancellationToken cancellationToken)
    {
        var fields = ValidateAndSanitize(request);
