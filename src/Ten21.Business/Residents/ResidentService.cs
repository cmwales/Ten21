...
/// <summary>
/// US-23/US-24: Tenant Profile Directory + Zero-Token Welcome & Provisioning, extracted from
/// ResidentsController. UserManager/RoleManager are ASP.NET Core Identity's own data/business
/// layer (already an Infrastructure dependency), so this service takes them directly the same
/// way it takes Ten21DbContext -- see OrganizationService's own comment for the same
/// reasoning. No repository -- every query here is a single-table find/list. No interface --
/// same reasoning as ChargeService/PaymentService.
///
/// CreateAsync provisions a real login for ANY resident with an email (primary or secondary),
/// via ProvisionResidentLoginAsync below.
/// </summary>
public class ResidentService
{
    /// <summary>
    /// US-24: hardcoded, not the configurable frontend-base-url pattern AuthController uses
    ...
    public async Task<IReadOnlyList<ResidentResponse>> GetResidentsAsync(Guid propertyId, CancellationToken cancellationToken)
    {
        ...
        return residents.Select(r => ToResponse(r, r.EmergencyContacts.ToList())).ToList();
    }

    public Task<ResidentProfile?> FindAsync(Guid propertyId, Guid id, CancellationToken cancellationToken) =>
        _dbContext.ResidentProfiles
            .Include(r => r.EmergencyContacts)
            .FirstOrDefaultAsync(r => r.PropertyId == propertyId && r.Id == id, cancellationToken);

    public static ResidentResponse BuildResponse(ResidentProfile resident) =>
        ToResponse(resident, resident.EmergencyContacts.ToList());

    public async Task<ResidentResponse> CreateAsync(
        Guid propertyId, UpsertResidentRequest request, CancellationToken cancellationToken)
    {
        await _dbContext.EnsurePropertyExistsAsync(propertyId, cancellationToken);
        ...
        // not dangerous: the resident record stays valid with no UserId, and CreateAsync
        // simply propagates the provisioning error to the caller.
        await ProvisionResidentLoginAsync(resident, cancellationToken);

        return ToResponse(resident, contacts);
    }

    public async Task<ResidentResponse> UpdateAsync(
        ResidentProfile resident, UpsertResidentRequest request, CancellationToken cancellationToken)
    {
        ValidateRequest(request);
        ...
        return ToResponse(resident, newContacts);
    }
    ...
    /// </summary>
    public async Task DeleteAsync(ResidentProfile resident, CancellationToken cancellationToken)
    {
    ...
    /// "one login, many tenant/role pairs" by design, so this links the existing account
    /// into this tenant rather than erroring or duplicating. Deliberately does NOT reset
    /// their password or set MustChangePassword -- they already have working credentials.
    /// Richer cross-PM UX (visibility across memberships, consent, conflict resolution) is
    /// deliberately deferred to a future sprint.
    /// </summary>
