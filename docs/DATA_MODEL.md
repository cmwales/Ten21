...
**For the complete, current list of every real table in the database** (all 21 domain
entities plus Identity/tenancy tables), see **`docs/DATABASE_SCHEMA.md`** — that doc is
generated by reading the actual entity classes and migrations, so it's the one to trust for
"does table X exist" and "what columns does it have today." This doc's job is different and
narrower: the *design rationale* behind the more intricate subsystems (why a table is shaped
the way it is, what it deliberately excludes and why) for the areas that have accumulated the
most nuance across sprints. It does not attempt to cover every table.

## 1. Financial & Payment Token Schema

### 1.1 PaymentMethods Table -- NOT YET BUILT (Phase 2)

This table does not exist in the codebase today. Live ACH/card payment collection (the
"tokenize via an external processor SDK, store only `pm_...` references" design in
`CLAUDE.md`/`SECURITY.md`) is Phase 2+ scope — see `ROADMAP.md`. `PaymentTransaction`
(§3.2 below) is what exists today, and it only ever records a *manually-entered* payment
received outside the app (cash, check, Zelle, Venmo, direct deposit) — no processor
integration, no stored payment instrument, tokenized or otherwise. When this table is
actually built, this section should be rewritten to describe it as shipped, not before.
