...
- Resource-based BOLA/IDOR authorization (`IAuthorizationService.EnsureSameTenantAsync`) stays in the controller: it resolves and authorizes the entity via a Service lookup method, then passes the already-authorized entity into subsequent Service calls.
