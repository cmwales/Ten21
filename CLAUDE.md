...
9. **Business layer (`Ten21.Business`):** project chain is Domain → Application → Infrastructure → Business → Api. Every controller action routes through a plain, concrete Business service — **controllers must never inject or query `Ten21DbContext` directly**; `Ten21.Api` only receives/sends data and handles genuinely HTTP-specific concerns (`ClaimsPrincipal`/`User.FindFirst`, `HttpContext`, `Request.Cookies`, `Response`, `IWebHostEnvironment`), extracted to plain values (`Guid userId`, `string? clientIp`, a raw cookie value, etc.) and passed into the service.
   * **No interface-per-class.** Services and repositories are concrete classes registered via `services.AddScoped<T>()` and injected concrete — an interface is only justified when there's a real second implementation or a genuine abstraction seam (e.g. `IInputSanitizer`, `IEmailSender`, `IJwtTokenService`, `IS3StorageService`, all in `Ten21.Application.Abstractions`). Never `new` up a service/repository outside DI in production code.
   * **`SaveChangesAsync` ownership:** the Business Service owns the unit of work. It accumulates all changes for one complete business operation and calls `SaveChangesAsync()` once at the end; repository/query classes must never call it themselves. Use an explicit `Database.BeginTransactionAsync` only when a genuine multi-save workflow needs atomicity, and only the Business Service manages that transaction boundary — never a repository.
   * **Repositories are the exception, not the default.** Don't create one solely to wrap `DbSet.Add`/`Remove`/a simple LINQ query — `Ten21DbContext` already provides that. A `*Repository` class earns its place only when it consolidates genuinely multi-table or batched queries reused as a unit (e.g. a statement's five parallel ledger queries, or a batch existence check across hundreds of rows). Repositories may live in `Ten21.Business` for now but should stay structured so they could move to a dedicated Infrastructure/Data project later without changing business behavior.
   * Identity types (`UserManager<ApplicationUser>`, `RoleManager<ApplicationRole>`) are legitimate direct Service dependencies, same tier as `Ten21DbContext` — they are not "HTTP-specific" the way `ClaimsPrincipal`/`HttpContext` are.
